...
class TestPrimeDecompMethods(unittest.TestCase):
  ...
  def test_prime_decomp_new_linear(self):
    d = (-7,-11,-19,-23)
    K = field.field(d)
    proof.number_field(False)
    ...
    for p in [163, 443, 463, 499, 653, 823, 883, 947]:
      if abs(idx) % p == 0:
        continue
      p_factors = prime_decomp.prime_decomp_new(d, p)
  ...
  def test_prime_decomp_new_full(self):
    fields = [
      (5, 13),
      (-11, -31),
      (-11, -19, -31),
    ...
    for d in fields:
      ...
      for p in primes(250):
        if abs(idx) % p == 0 or Mod(disc, p) == 0:
          continue
        D1 = prime_decomp.prime_decomp_new(d, p)
        D1 = [(K2.ideal(I[0][0], K2(str(I[0][1].to_sage(names='a')))), I[1]) for I in D1]
        D2 = K2.factor(p)
  ...
  def test_prime_decomp_new_full_not_prime_di(self):
    fields = [
       (-3, 5*17),
       (7*11, 23*43),
       (3*5, 7*13, 3*13)
  ...
  def test_prime_decomp_new_gammas(self):
