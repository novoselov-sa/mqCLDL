...
class TestTreesNewSecFieldMethods(unittest.TestCase):
  def test_ideal_up_sec_field(self):
    ...
    for p in primes:
      ...
      for I,e in F:
        #D1 = trees.ideal_up(d_K, d_L, I, aut=True)
        I_up = trees.ideal_up(d_K, d_L, I)
        #print(f"I_up[0] = {I_up[0][1].to_sage(names='a')}, {I_up[0][1].evaluate_mod(p)}, {I_up[0][1].evaluate_mod_ext(p)}")
        #print(f"I_up[1] = {I_up[1][1].to_sage(names='a')}, {I_up[1][1].evaluate_mod(p)}, {I_up[1][1].evaluate_mod_ext(p)}")
